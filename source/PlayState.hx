...
class PlayState extends MusicBeatState
{
	...
	var updikeglow:Character;
	var garglow:Character; 
	var garghost:Boyfriend; 
	var Edgy:Bool = false;
	var Gargon:Bool = false;


	var edgybg:FlxSprite;
	...
	override public function create()
	{
			...
			case 'room':
				{
					...
					/*edgybg = new FlxSprite(600, -200).loadGraphic(Paths.image('edgywall'));
					edgybg.antialiasing = true;
					edgybg.scrollFactor.set(0.9, 0.9);
					edgybg.active = false;
					edgybg.setGraphicSize(Std.int(bg.width * 1.3));
					edgybg.alpha = 0;
					add(edgybg);*/

					bg = new FlxSprite(300, -200).loadGraphic(Paths.image('wallbg'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
		...
		garglow = new Character(770, 450, 'garglow');
		garghost = new Boyfriend(770, 450, 'garghost');
		updikeglow = new Character(100, 100,'updikeglow');
		garglow.alpha = 0;
		garghost.alpha = 0;
		updikeglow.alpha = 0;
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						switch (Math.abs(daNote.noteData))
						{
							case 2:
								if(Edgy) 
									updikeglow.playAnim('singUP' + altAnim, true);
								else
									dad.playAnim('singUP' + altAnim, true);
							case 3:
								if(Edgy)
									updikeglow.playAnim('singRIGHT' + altAnim, true);
								else
									dad.playAnim('singRIGHT' + altAnim, true);
							case 1:
								if(Edgy)
									updikeglow.playAnim('singDOWN' + altAnim, true);
								else
									dad.playAnim('singDOWN' + altAnim, true);
							case 0:
								if(Edgy)
									updikeglow.playAnim('singLEFT' + altAnim, true);
								else
	...
	function Edgyone():Void //This Event Is Fun to make :) -TaeYai
		{
			if(Edgy)
			{
				//What? There Are gone
	...
	*/
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					{
						case 2:
								if(Edgy) //Im very tired to make this - TaeYai
									garglow.playAnim('singUP', true);
								else
									boyfriend.playAnim('singUP', true);
								    garghost.playAnim('singUP', true);
							case 3:
								if(Edgy)
									garglow.playAnim('singRIGHT', true);
								else
									boyfriend.playAnim('singRIGHT', true);
								    garghost.playAnim('singRIGHT', true);
							case 1:
								if(Edgy)
									garglow.playAnim('singDOWN', true);
								else
									boyfriend.playAnim('singDOWN', true);
								    garghost.playAnim('singDOWN', true);
							case 0:
								if(Edgy)
									garglow.playAnim('singLEFT', true);
								else
									boyfriend.playAnim('singLEFT', true);
								    garghost.playAnim('singLEFT', true);
					}
	...
	override function stepHit()
	{
		...
		if (curStage == 'room' && curStep != stepOfLast)
			{
				switch(curStep)
				{
					case 384:
